...
def handle_create_template(event, context):
    ...
    try:
        ...
        required_fields = ["template_short_name", "template_description", "template_prompt"]
        for field in required_fields:
            if not body.get(field):
                return CORSResponse.error_response(f"Missing required field: {field}", 400)
        ...
        bedrock_kwargs = body.get("bedrock_kwargs", {"temperature": 0.1, "max_tokens": 2000})
...
def handle_update_template(event, context):
    ...
    try:
        ...
        existing_template = get_template_by_id(analysis_templates_table, username, template_id)
        ...
        # Update the template
        template = update_user_template(analysis_templates_table, username, template_id, body)
...
def handle_delete_template(event, context):
    ...
    try:
        ...
        return CORSResponse.success_response({"message": "Template deleted successfully"})
